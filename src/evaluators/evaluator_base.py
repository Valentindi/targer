...
class EvaluatorBase():
    ...
    def get_evaluation_score_train_dev_test(self, tagger, datasets_bank, batch_size=-1, verbose=False):
        if batch_size == -1:
    ...
    def predict_evaluation_score(self, tagger, word_sequences, targets_tag_sequences, batch_size,verbose=False):
        outputs_tag_sequences = tagger.predict_tags_from_words(word_sequences, batch_size)
        acc, msg = self.get_evaluation_score_train_dev_test(targets_tag_sequences, outputs_tag_sequences, word_sequences)
        if verbose:
